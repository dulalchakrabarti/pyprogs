...
class Transformer(nn.Module):
    def __init__(self, feature_size, output_size, num_encoder_layers, num_heads, num_decoder_layers, device,
        ...
        decoder_layer = nn.TransformerDecoderLayer(d_model=feature_size, nhead=num_heads,
                                                   dim_feedforward=dim_feedforward, dropout=dropout, device=device,
                                                   batch_first=batch_first)
    ...
    def forward(self, enc_input, dec_input):
        ...
        src_pos_enc = enc_input + self.positional_encoding(seq_len=enc_input.shape[1], dim_model=enc_input.shape[2],
                                                           device=self.device)
        src_pos_dec = dec_input + self.positional_encoding(seq_len=dec_input.shape[1], dim_model=dec_input.shape[2],
                                                           device=self.device)
...
def training_transformer(model, optimizer, criterion, train_loader, test_loader, n_epochs, train_loss=None,
                         test_loss=None):
    if train_loss is not None:
        epoch_loss_train = train_loss
        best_train_loss = min([np.mean(i) for i in train_loss])
...
def define_train_transformers(models, device, dataset, training_results_transformers, path_save, colab):
    for i in models:
        if models[i][7] is True:
            ...
            torch.save(Transformer_trained_Model, path_save + '/Transformer_' + str(models[i][8]) + '.pt')
