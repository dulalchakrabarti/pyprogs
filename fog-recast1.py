...
def count_out_ir1(lo, la):
    ...
    try:
     ...
     print('R-Timed out!!')
    return ser
gl = open(date1_[2]+date1_[1]+date1_[0]+'fog_ir1.csv','w')
count = 0
d = {}
 ...
 if len(buf) != 6:
  ...
  for key in klst:
   if ts[key] == '' or ts[key] == None:
    ...
    print(lon,lat,stamp(key),ts[key])
    gl.write(str(lon)+','+str(lat)+','+stamp(key)+','+str(ts[key])+'\n')
  count+=1
seconds2 = time.time()
print('elapsed minutes....',(seconds2-seconds1)/60.0,count)
gl.close()
print('Starting again for ir2 chaannel....')
txt = date1+'T00:00:00.000Z'+','+date1+'T00:15:00.000Z'+','+ date1+'T00:30:00.000Z'+','+ date1+'T00:45:00.000Z'+','+\
date1+'T01:00:00.000Z'+','+date1+'T01:15:00.000Z'+','+ date1+'T01:30:00.000Z'+','+ date1+'T01:45:00.000Z'+','+\
date1+'T02:00:00.000Z'+','+date1+'T02:15:00.000Z'+','+ date1+'T02:30:00.000Z'+','+ date1+'T02:45:00.000Z'+','+\
date1+'T03:00:00.000Z'+','+date1+'T03:15:00.000Z'+','+ date1+'T03:30:00.000Z'+','+ date1+'T03:45:00.000Z'+','+\
...
def count_out_ir2(lo, la):
    ...
    urla = la + dlat
    try:
     txt = "https://rapid.imd.gov.in/r2wms/wms?&service=WMS&request=GetTimeseries&version=1.3.0&CRS=CRS:84&BBOX="+str(lllo)+","+str(llla)+","+str(urlo)+","+str(urla)+"&I=362&J=294&TIME="+date1+"T"+time1+"Z/"+date1+"T"+time2+"Z&INFO_FORMAT=text/json&ELEVATION=0&QUERY_LAYERS=3DIMG_L1B_STD4/IMG_TIR2&LAYERS=3DIMG_L1B_STD4/IMG_TIR2&FEATURE_COUNT=1&WIDTH=724&HEIGHT=588"
     #print(txt)
     r1 = requests.get(txt)
     resp = r1.text
     #print(resp)
     ...
     la_ = str(data['domain']['axes']['y']['values'][0])
     sat ='D'
     dval = data['ranges']['IMG_TIR2']['values']
     dtime = data['domain']['axes']['t']['values']
     keyid = str(lo)+'_'+str(la)+'_'+sat
     ser.append(keyid)
     ser.append(dtime)
     ...
     print('D-Timed out!!')
    try:
     txt = "https://rapid.imd.gov.in/r2wms/wms?&service=WMS&request=GetTimeseries&version=1.3.0&CRS=CRS:84&BBOX="+str(lllo)+","+str(llla)+","+str(urlo)+","+str(urla)+"&I=362&J=294&TIME="+date1+"T"+time1+"Z/"+date1+"T"+time2+"Z&INFO_FORMAT=text/json&ELEVATION=0&QUERY_LAYERS=3RIMG_L1B_STD4/IMG_TIR2&LAYERS=3RIMG_L1B_STD4/IMG_TIR2&FEATURE_COUNT=1&WIDTH=724&HEIGHT=588"
     #print(txt)
     r1 = requests.get(txt)
     resp = r1.text
     #print(resp)
     ...
     la_ = str(data['domain']['axes']['y']['values'][0])
     sat ='R'
     rval = data['ranges']['IMG_TIR2']['values']
     rtime = data['domain']['axes']['t']['values']
     keyid = str(lo)+'_'+str(la)+'_'+sat
     ser.append(keyid)
     ser.append(rtime)
     ...
     print('R-Timed out!!')
    return ser
gl = open(date1_[2]+date1_[1]+date1_[0]+'fog_ir2.csv','w')
count = 0
for key in keylist:
 rad = d[key].split(',')
 lat = rad[0]
 lon = rad[1]
 buf = count_out_ir2(lon,lat)
 if len(buf) != 6:
   buf = count_out_ir2(lon,lat)
 else:
  dts = buf[1:3]
