...
def count_out(lon,lat):
    ...
    for lati in np.arange(la_st_,la_end_,dy/64):
     lati_ = str(lati)
     try:
      r1 = requests.get("https://rapid.imd.gov.in/r2wms/wms?&service=WMS&request=GetTransect&LAYERS=3DIMG_L1B_STD4/IMG_MIR&CRS=CRS:84&LINESTRING="+lo_st+"%20"+lati_+","+lo_end+"%20"+lati_+",&TIME=2022-02-01T18:00:00&FORMAT=text/json")
      resp = r1.text
      data = json.loads(resp)
      lat = data['lat'][25:40]
      lon = data['lon'][25:40]
      vals = data['IMG_MIR']['values'][25:40]
